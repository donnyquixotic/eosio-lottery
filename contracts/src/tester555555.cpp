...
class [[eosio::contract("tester555555")]] tester555555 : public contract {  
    private: 
        ...
        const uint32_t MONTH_SECONDS = 2629743;
        const uint32_t HOUR_SECONDS = 3600;
        const uint32_t MINUTE_SECONDS= 60;
        const int64_t DIVISOR = 10000;
        const name ORACLE_CONTRACT = name("rng.oracle");
    ...
    public:    
        ...
        void endlotto(){      
            ...
            //if no entries at end of open period, reset and re-open
            if (itr->max == 0){
                lotteries.modify(itr,get_self(), [&]( auto &e) {
                    e.end_lottery = new_end();
                    e.is_open = true;
            ...
            uint64_t seed = 76543254;
            uint64_t request_id = 456;
        ...
        void deposit(name player, name to, asset quantity, string memo) {   
            ...
            check( current_time < itr->end_lottery,"lottery has closed" );
            check( itr->is_open,"current lottery is closed, try again");
            check(quantity.amount > 0, "In order to participate you must send at least 1.0000 TLOS");      
        ...
        void receiverand(uint64_t caller_id, checksum256 random) {
            ...
            uint64_t number = random_int % ( max + 1 ) ;
